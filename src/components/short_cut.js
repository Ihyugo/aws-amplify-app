copy_and_paste = [['Ctrl + X','選択した項目を切り取る。'],
...
win_log_short = [
    ...
    ['Windows ロゴ キー  + C','Microsoft Teams から [チャット] を開きます。 Windows 11 で更新されました。'],
    ['Windows ロゴ キー  + Shift + C','チャーム メニューを開く。'],
    ['Windows ロゴ キー  + Ctrl + C','カラー フィルターを有効にします ([カラー フィルターの設定] で最初にこのショートカットを有効にします)。'],
    ['Windows ロゴ キー  + D','デスクトップを表示または非表示にする。'],
    ...
    ['Windows ロゴ キー  + K','[クイック スタート] から [キャスト] を開きます。 Windows 11 で更新されました。'],
    ...
    ['Windows ロゴ キー  + Alt + R','(Xbox Game Bar を使用して) フォーカスがあるゲーム ウィンドウのビデオを録画します。'],
    ...
    ['Windows ロゴ キー  + Shift + 数字','デスクトップを開き、タスク バー上の数字で示す位置にピン留めされているアプリの新しいインスタンスを開始する。'],
    ['Windows ロゴ キー  + Ctrl + 数字','デスクトップを開き、タスク バー上の数字で示す位置にピン留めされているアプリの前回の作業中のウィンドウに切り替える。'],
    ['Windows ロゴ キー  + Alt + 数字','デスクトップを開き、タスク バー上の数字で示す位置にピン留めされているアプリのジャンプ リストを開く。'],
    ['Windows ロゴ キー  + Ctrl + Shift + 数字','デスクトップを開き、タスク バー上の指定された位置にあるアプリの新しいインスタンスを管理者として開く。'],
    ['Windows ロゴ キー  + Tab','タスク ビューを開く。'],
    ['Windows ロゴ キー  + ↑','ウィンドウを最大化する。'],
    ['Windows ロゴ キー  + Shift + ↑','画面の上半分にフォーカスがあるウィンドウをスナップします。 Windows 11 の新機能。'],
    ['Windows ロゴ キー  + ↓','現在のアプリを画面から削除する、またはデスクトップ ウィンドウを最小化する。'],
    ['Windows ロゴ キー  + Shift + ↓','画面の下半分にフォーカスがあるウィンドウをスナップします。 Windows 11 の新機能。'],
    ['Windows ロゴ キー  + ←','画面の左側にアプリまたはデスクトップ ウィンドウを最大化する。'],
    ['Windows ロゴ キー  + →','画面の右側にアプリまたはデスクトップ ウィンドウを最大化する。'],
    ['Windows ロゴ キー  + Home','作業中のデスクトップ ウィンドウを除くすべてのウィンドウを最小化する (2 回目のキーストロークですべてのウィンドウを復元する)。'],
    ...
    ['Windows ロゴ キー  + Ctrl + Shift + B','PC のブランク画面またはブラック スクリーンから復帰します。'],
    ['Windows ロゴ キー  + Prt Scr','全画面のスクリーンショットをファイルに保存します。'],
    ['Windows ロゴ キー  + Alt + Prt Scr','(Xbox Game Bar を使用して) フォーカスがあるゲーム ウィンドウのスクリーンショットをファイルに保存します。'],
]
